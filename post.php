...
<ul class="content rt rt-13">
    ...
    $blogm = mysqli_query($con,"SELECT id,`name` FROM `mnue` where idmaker='$idsender'");
    while( $blognamem=mysqli_fetch_array($blogm)){
    $blognamem1=$blognamem['id'];
    $blognamem2=$blognamem['name'];
        $blognum = mysqli_query($con,"SELECT * FROM `blog` WHERE `idmnue`='$blognamem1'");
        $numb=mysqli_num_rows($blognum);

    ?>

        <li class="rt"><a href="blog.php?idm=<?php echo $blognamem1; ?>" class="right"><i class="fa fa-caret-left"></i><?php echo $blognamem2; ?></a><span class="left rt-12"><?=$numb?></span></li>
    <?php
    }         ?>
...
<ul class="content lsat-blog-posts rt rt-13">
    ...
    $rezal = mysqli_query($con,"select * from blog  where idsender='$idsender' ORDER BY id  DESC LIMIT 0 , 5");
