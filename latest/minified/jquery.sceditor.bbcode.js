	...
	// TODO: move these styles into the CSS file
			...
			aparatMenu:
				'<div><label for="link">{label}</label> ' +
					'<input type="text" id="link" placeholder="http://www.aparat.com/v/6rQNZ" /></div>' +
				'<div><input type="button" class="button" value="{insert}" />' +
					'</div>',

			aparat:
            '<iframe src="https://www.aparat.com/video/video/embed/videohash/{id}/vt/frame" height="360" width="100%"style="border: none;" allowFullScreen="true" webkitallowfullscreen="true" mozallowfullscreen="true" data-aparat-id="{id}"></iframe>'
		};
	...
	// Maybe just check the firstBlock contains both the start and end containers
			...
			aparat: {
				...
				exec: function (caller) {
					...
					defaultCommnds.aparat._dropDown(
						editor,
						caller,
						function (id) {
							editor.wysiwygEditorInsertHtml(_tmpl('aparat', {
								id: id
							}));
						}
					);
				},
				tooltip: 'Insert a aparat video'
			},
			// END_COMMAND
	...
	var __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
		...
		return {
			...
			toolbar: 'bold,italic,underline,strike,subscript,superscript|' +
				...
				'emoticon,aparat,date,time|ltr,rtl|print,maximize,source',
	...
	var defaultCommandsOverrides = {
		...
		aparat: {
			txtExec: function (caller) {
				...
				getEditorCommand('aparat')._dropDown(
					editor,
					caller,
					function (id) {
						editor.insertText('[aparat]' + id + '[/aparat]');
					}
				);
			}
		},
...
// instead by creating BBCode tokens like the parser.
	...
	var bbcodes = {
		...
		aparat: {
			...
			format: function (element, content) {
				element = element.attr('data-aparat-id');

				return element ? '[aparat]' + element + '[/aparat]' : content;
			},
			html: '<iframe src="https://www.aparat.com/video/video/embed/videohash/{0}/vt/frame" height="360" width="100%"style="border: none;" allowFullScreen="true" webkitallowfullscreen="true" mozallowfullscreen="true" data-aparat-id="{0}"></iframe>'
